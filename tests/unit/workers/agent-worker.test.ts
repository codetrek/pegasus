 ...
 * handleMessage, handleLLMResponse, handleLLMError, handleShutdown,
 * loadPreviousTaskSummary) via _testState and postMessage overrides.
 *
 ...
 * Bun test run.
 *
 * Functions that construct a real Agent (initProject, initSubAgent, handleInit)
 * are covered by integration tests in tests/integration/agent-worker.test.ts
 * which spawn real Worker threads.
 */
