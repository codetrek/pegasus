/**
 * Unit tests for agent-worker.ts exported functions.
 *
 ...
 * initProject, initSubAgent) by mocking module-level state and postMessage.
 *
 * The Agent and ProxyLanguageModel constructors are mocked via bun:test mock.module
 * to avoid creating real instances that would leave background resources open.
 *
...
// ── Mock Agent and ProxyLanguageModel before importing agent-worker ──
// This prevents real Agent instances from being created, which would leave
// background resources (EventBus, timers) open and cause process hangs.
...
// Use bun's mock.module to replace Agent and ProxyLanguageModel imports
// in agent-worker.ts so no real Agent instances are created.
mock.module("@pegasus/agents/agent.ts", () => ({
  Agent: class MockAgent {
    constructor() {
      Object.assign(this, lastMockAgent);
    }
  },
}));

mock.module("@pegasus/projects/proxy-language-model.ts", () => ({
  ProxyLanguageModel: class MockProxy {
    provider: string;
    modelId: string;
    constructor(provider: string, modelId: string) {
      this.provider = provider;
      this.modelId = modelId;
      Object.assign(this, lastMockProxy);
    }
  },
}));
...
describe("handleInit", () => {
  ...
  afterEach(() => {
    ...
    _testState.setAgent(null);
    _testState.setProxyModel(null);
    // Restore settings in case resetSettings() was called
...
describe("initSubAgent", () => {
  ...
  it("should auto-submit input when non-empty", async () => {
    ...
    expect(messages).toContainEqual({ type: "ready" });
    // When input is non-empty, agent.submit() is called which triggers an LLM request
    // The agent is a mock, so just verify ready was sent (submit behavior tested in integration)
  }, 10_000);
  ...
  it("should prepend memorySnapshot to input when provided", async () => {
    ...
    expect(messages).toContainEqual({ type: "ready" });
    // memorySnapshot and input behavior is verified by integration tests
