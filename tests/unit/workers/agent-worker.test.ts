 ...
 * initProject) via _testState, postMessage overrides, and factory overrides.
 *
...
describe("handleInit", () => {
  ...
  it("should catch errors and post error message for project mode (no settings)", async () => {
    // Reset settings so getSettings() will throw
    resetSettings();

    await handleInit("project", {
      projectPath: `${TEST_DIR}/no-settings`,
    });
...
// ── initSubAgent tests removed — SubAgentManager/Worker subagent mode deleted ──
